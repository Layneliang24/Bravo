...
function verifyCoverage() {
  ...
  if (!fs.existsSync(COVERAGE_DIR)) {
    console.error(
      '❌ Coverage directory not found. Tests may not have run properly.'
    )
    process.exit(1)
  }
  ...
  if (!fs.existsSync(COVERAGE_SUMMARY_FILE)) {
    console.error(
      '❌ Coverage summary file not found. Coverage may not have been collected.'
    )
    process.exit(1)
  }
  ...
  if (totalCoverage.lines.pct < COVERAGE_THRESHOLDS.global.lines) {
    failures.push(
      `Lines: ${totalCoverage.lines.pct}% < ${COVERAGE_THRESHOLDS.global.lines}%`
    )
  }

  if (totalCoverage.statements.pct < COVERAGE_THRESHOLDS.global.statements) {
    failures.push(
      `Statements: ${totalCoverage.statements.pct}% < ${COVERAGE_THRESHOLDS.global.statements}%`
    )
  }

  if (totalCoverage.functions.pct < COVERAGE_THRESHOLDS.global.functions) {
    failures.push(
      `Functions: ${totalCoverage.functions.pct}% < ${COVERAGE_THRESHOLDS.global.functions}%`
    )
  }

  if (totalCoverage.branches.pct < COVERAGE_THRESHOLDS.global.branches) {
    failures.push(
      `Branches: ${totalCoverage.branches.pct}% < ${COVERAGE_THRESHOLDS.global.branches}%`
    )
  }
  ...
  if (fileCount < 5) {
    console.warn(
      `⚠️  Warning: Only ${fileCount} files in coverage report. This seems low.`
    )
  }
  ...
  if (suspiciousFiles.length > fileCount * 0.8) {
    console.warn(
      '⚠️  Warning: High percentage of files with 100% coverage. Please verify test quality.'
    )
  }
