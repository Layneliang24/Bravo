...
global.afterAll(() => {
  ...
  if (process.env.CI && !fs.existsSync(coverageFile)) {
    console.error('❌ ANTI-CHEATING: Coverage file not found - tests may not have run properly');
    process.exit(1);
  }
  ...
  if (fs.existsSync(coverageFile)) {
    ...
    if (fileCount === 0) {
      console.error('❌ ANTI-CHEATING: No coverage data collected - suspicious test run');
      process.exit(1);
    }
...
module.exports = {
  verifyCoverageIntegrity: () => {
    ...
    if (!fs.existsSync(coverageFile)) {
      throw new Error('Coverage file not found - tests may not have executed properly');
    }
    ...
    Object.values(coverage).forEach(file => {
      if (file.s) {
        ...
        summary.coveredLines += Object.values(file.s).filter(count => count > 0).length;
      }
    });
    
    const overallCoverage = summary.totalLines > 0 
      ? (summary.coveredLines / summary.totalLines) * 100 
      : 0;
    
    console.log(`📊 Coverage Summary: ${overallCoverage.toFixed(2)}% (${summary.coveredLines}/${summary.totalLines} lines)`);
