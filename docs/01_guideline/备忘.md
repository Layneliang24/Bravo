...
- `golden_test_protection.mdc` - Cursor AI行为规则
...
已经成功实现了完整的测试锁定系统，确保代码质量和功能完整性：

##### ✅ 已完成的核心功能

1. 强制全量测试CI配置

- 配置了 `ci.yml` GitHub Actions工作流
- 实现强制运行所有测试（前端、后端、E2E）
- 自动上传测试报告和覆盖率数据
- 测试失败时阻止合并

2. 覆盖率阈值锁定

- 在 `jest.config.js` 中设置90%硬性覆盖率阈值
- 配置详细的覆盖率报告生成
- 覆盖率不达标时自动失败
...
- 配置了dev分支保护规则
- 强制要求PR通过所有测试检查
- 实现了完整的回归测试套件 `test_regression.py`
- 创建了独立的测试运行器 `test_simple.py`

4. 功能-测试映射系统

- 建立了结构化的 `features.json` 功能清单
- 实现了 `linkTestToFeature` 映射机制
- 创建了 `testMap.js` 测试映射工具
...
- 更新了 `system_prompt.md`
- 添加了功能-测试映射锁约束
- 强化了代码质量检查规则

##### 🛡️ 安全保障机制

- 多层防护 ：CI/CD + 分支保护 + 覆盖率阈值
- 自动化检查 ：所有提交都必须通过完整测试套件
- 质量门禁 ：90%覆盖率硬性要求
- 功能追踪 ：每个功能都有对应的测试映射
...
- 更新了 `test_system_verification_report.md`
- 测试体系完成度提升至 95%
- 仅变异测试因Windows平台限制未完全实现

##### 维护建议

定期运行 python health_check_complete.py 进行测试体系健康检查，确保系统持续稳定运行。测试体系现已具备完整的故障恢复能力和健壮性验证机制。
...
- 创建了 `test_all.sh` 作为一键测试入口
- 更新了 `Makefile` 添加 test-all 命令
- 更新了 `package.json` 添加 test:all 脚本

2. 分层任务编排

- 后端测试 : pytest 自动收录 test\_\*.py 文件
- 前端测试 : Vitest/Jest 自动收录 \*.test.js 文件
- E2E测试 : Playwright 自动收录 \*.spec.ts 文件
- 回归测试 : 自动执行回归测试脚本
- 性能测试 : Lighthouse CI 集成
- 代码质量 : ESLint + Pylint 检查

3. 自动收录机制

- 后端 : backend/tests/\*\_/test\_\_.py (pytest 自动收集)
- 前端 : frontend/tests/\*_/_.test.js (Vitest 自动收集)
- E2E : e2e/tests/\*_/_.spec.ts (Playwright 自动收集)
- 零配置 : 新增测试文件遵循命名规范即可自动被发现
...
### cursor排查问题时为了让测试通过，进行一堆简化，或者删除代码或者是注释掉一些代码，后续又没有还原回来，以后可能导致功能减少甚至目标偏离，这个怎么解决？
...
文件 : scripts/fast_pre_commit.py

- 类 : FastPreCommitChecker - 高性能代码扫描引擎
- 技术栈 : Python + 正则表达式 + Git Python API
- 核心功能 :
  - 增量文件扫描（仅检查修改的文件）
  - 多线程正则匹配
  - 智能严重性分级
...
文件 : scripts/production_config.py
...
# 严重性阈值配置
SEVERITY_THRESHOLDS = {
    'block_commit': True,          # 是否
    阻止提交
    'max_high_severity': 0,        # 高严
    重性最大允许数
    'max_medium_severity': 3,      # 中严
    重性最大允许数
    'max_low_severity': 5         # 低严重
    性最大允许数
}
```
...
- 统计收集 : 总检查次数、阻止提交数、平均扫描时间
- 性能监控 : 内存使用、扫描效率、错误率
- 健康检查 : Git钩子状态、配置文件完整性、Python环境
...
- 增量扫描 : 仅检查 git diff --cached 中的修改文件
- 并行处理 : 使用正则表达式引擎的C扩展
- 缓存机制 : 统计信息本地缓存，减少重复计算
...
- 自定义规则 : 通过修改 production_config.py 添加新的检测模式
- 阈值调整 : 灵活配置不同严重性的阻止策略
- 多语言支持 : 支持 Python、JavaScript、TypeScript、Vue 等文件类型
  系统已完全部署并投入生产使用，为 Bravo 项目提供了可靠的代码质量保障。
