# workflow_run headBranch 问题分析

## 问题描述

自动部署工作流失败，使用了旧版本的工作流文件（commit `7242a7661d83e910e4f40b34740c6180d638d0cc`），导致部署脚本仍在执行 `git clone` 等旧逻辑。

## 根本原因

### 1. workflow_run 触发机制

`workflow_run` 触发时，GitHub Actions 的行为：

1. **工作流文件版本确定时机**：工作流文件本身在触发时确定，使用的是触发工作流（构建工作流）的 `headBranch` 和 `headSha`
2. **headBranch 继承**：如果构建工作流从 `main` 分支触发，部署工作流的 `headBranch` 也会是 `main`
3. **branches 限制不足**：虽然设置了 `branches: [dev]`，但这只限制触发条件，不保证工作流文件使用 dev 分支的版本

### 2. 实际发生的情况

从运行记录分析：

- 构建工作流（20120249577）：`headBranch: "dev"`，成功
- 自动部署工作流（20120288322）：`headBranch: "main"`，`headSha: "7242a76"`（旧commit），失败

**问题**：虽然构建工作流从 dev 分支触发，但自动部署工作流的 `headBranch` 却是 `main`，说明：

- 可能有其他构建工作流从 main 分支触发，导致部署工作流被错误触发
- 或者 `workflow_run` 的 `branches: [dev]` 限制没有正确工作

## 解决方案

### 方案1：在 if 条件中检查 head_branch（已实施）

```yaml
if: ${{ github.event_name == 'workflow_dispatch' || (github.event.workflow_run.conclusion == 'success' && github.event.workflow_run.head_branch == 'dev') }}
```

**优点**：

- 确保只有从 dev 分支触发的构建工作流才会触发部署
- 即使 `workflow_run` 被错误触发，也会在 if 条件中被过滤

**缺点**：

- 如果构建工作流从 main 分支触发，部署工作流不会执行（这是期望的行为）

### 方案2：确保构建工作流只从 dev 分支触发

检查 `build-and-push-images.yml` 的触发条件，确保：

- 只有 dev 分支的 push 才会触发构建
- 或者，在构建工作流中添加分支检查

### 方案3：使用 workflow_call 替代 workflow_run

`workflow_call` 可以更精确地控制触发条件，但需要修改构建工作流，在构建完成后显式调用部署工作流。

## 验证方法

1. **检查构建工作流的触发分支**：

   ```bash
   gh run list --workflow="🐳 Build and Push Docker Images" --limit 10 --json headBranch,event
   ```

2. **检查部署工作流的触发来源**：

   ```bash
   gh run view <run-id> --json event,headBranch,headSha
   ```

3. **验证修复效果**：
   - 触发一次构建工作流（从 dev 分支）
   - 检查自动部署工作流是否使用正确的 headBranch 和 headSha

## 参考文档

- [GitHub Actions: workflow_run](https://docs.github.com/en/actions/using-workflows/events-that-trigger-workflows#workflow_run)
- [GitHub Actions: workflow_run context](https://docs.github.com/en/actions/learn-github-actions/contexts#workflow_run-context)
