...
**时间线**：2024年12月7日下午开始，持续到晚上
**核心问题**：dev分支无法合并到main分支，反复出现冲突和CI检查失败
**最终状态**：已成功合并到dev和main
...
3. **`scripts/protect_golden_scripts.py`代码差异**：
   - dev分支有更新的`get_actually_modified_files()`函数
   - 这是正常的代码改进，不是自动变更
   - **解决方案**：选择dev版本合并到main
...
3. **`test-suite.yml`条件逻辑问题**：
   - `integration-tests`和`e2e-tests`的条件在直接PR触发时失败
   - **解决方案**：修改条件逻辑，处理`inputs.test-level`为null的情况
...
3. **循环依赖**：
   ```
   dev和main冲突 → 创建PR → PR触发时间戳更新 → 新的冲突 → 需要新的PR → ...
   ```
...
3. **缺少本地验证**：
   - `act`工具可以验证语法，但无法验证触发逻辑
   - 没有在本地测试PR触发场景
   - **何时埋下隐患**：依赖GitHub的远程验证，没有建立完整的本地验证流程
...
2. **IndentationError**：
   - `scripts-golden/local_test_passport.py`第284行缺少缩进
   - `.compliance/checkers/test_checker.py`第75行缺少缩进
   - **原因**：之前的编辑操作破坏了缩进
...
3. **岔路3：等待CI检查自动运行**
   - **原因**：没有检查工作流配置，假设会自动触发
   - **教训**：主动验证配置，不要假设系统会自动工作
...
if git rev-parse --verify MERGE_HEAD >/dev/null 2>&1; then
    echo "⏭️  检测到merge操作，跳过时间戳更新"
    exit 0
fi
```

### 规则4：工作流必须同时支持push和pull_request
...
1. **理解工具限制**：GitHub API不能解决所有问题，必须理解Git的基本机制
2. **系统性分析**：不要只修复症状，要找出根本原因
3. **完整验证**：本地验证必须覆盖所有场景（push、PR、merge）
4. **配置一致性**：分支保护规则必须与工作流配置保持一致
5. **预防胜于修复**：在开发阶段就预防冲突，而不是事后解决

### 改进措施

1. ✅ 修复了git保护脚本的merge场景处理
2. ✅ 为所有工作流添加了pull_request触发器
3. ✅ 添加了专门的run-tests job匹配分支保护规则
4. ✅ 修复了test-suite.yml的语法错误
5. ✅ 改进了本地测试通行证的验证逻辑

### 待实施措施

1. ⏳ 建立工作流语法验证的pre-commit hook
2. ⏳ 建立分支保护规则与工作流的一致性检查
3. ⏳ 实现自动化冲突解决助手
4. ⏳ 建立预合并验证工作流
5. ⏳ 实现智能合并策略
