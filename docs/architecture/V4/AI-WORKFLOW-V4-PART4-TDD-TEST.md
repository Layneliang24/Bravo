...
- [2. 测试用例设计阶段（新增）](#2-测试用例设计阶段新增)
- [3. 四层测试体系](#3-四层测试体系)
- [4. 测试目录强制规范](#4-测试目录强制规范)
- [5. TDD工作流](#5-tdd工作流)
- [6. 测试文件与代码文件关联](#6-测试文件与代码文件关联)
- [7. 测试验证与覆盖率](#7-测试验证与覆盖率)
- [8. 测试失败处理](#8-测试失败处理)
- [9. 回归测试机制](#9-回归测试机制)
...
> **V4.1更新**: 增加测试用例设计与评审阶段，确保测试完整性

### 2.1 为什么需要测试用例设计阶段

**问题分析**:

传统TDD流程中，开发者直接根据PRD编写测试代码。这存在以下问题：

1. **测试设计不系统**: 容易遗漏场景
2. **缺乏评审环节**: 无法提前发现测试遗漏
3. **可追溯性不足**: 测试与PRD功能点的映射不清晰
4. **难以评估覆盖率**: 无法在编码前评估测试完整性

**解决方案**:

在TDD红色阶段之前，增加测试用例设计和评审阶段：
...
**文件位置**: `docs/00_product/requirements/{REQ-ID}/{REQ-ID}-test-cases.csv`

**必填字段**:

| 字段       | 说明                            | 示例                    |
| ---------- | ------------------------------- | ----------------------- |
| 用例ID     | 唯一标识符                      | TC-AUTH-001             |
| 用例名称   | 简短描述                        | 用户成功登录            |
...
- 测试数据、边界条件、异常场景、UI验证点、性能要求、可访问性
- API相关：HTTP方法、API路径、请求体、预期状态码、预期响应

**CSV模板**: `docs/templates/testcase-template.csv`
...
**评审检查清单**: `docs/templates/testcase-review-checklist.md`
...
| ----------- | -------------------------------------------- | --------------------------------- |
| UNIT        | backend/tests/unit/test\_{module}.py         | test*{用例ID}*{场景}              |
| INTEGRATION | backend/tests/integration/test\_{feature}.py | test*{用例ID}*{场景}              |
| E2E         | e2e/tests/{feature}.spec.ts                  | test('{用例ID}: {用例名称}', ...) |

**详细规则**: 参考 `.cursor/rules/workflows/testcase-design.mdc`
