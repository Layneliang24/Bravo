            ...
            try:
                with open(path, "r", encoding="utf-8") as f:
                    ...
                    if content and "on" in content and "workflow_call" in content["on"]:
                        print("   ↳ 支持workflow_call")
                    else:
                        print("   ⚠️ 不支持workflow_call")
...
def check_cache_strategy():
    ...
    if os.path.exists(cache_file):
        ...
        cache_steps = [
            step for step in steps if "cache" in step.get("name", "").lower()
        ]
        install_steps = [
            step for step in steps if "install" in step.get("name", "").lower()
        ]
