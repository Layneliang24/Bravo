...
class ApiRegressionTester {
  ...
  async runApiTest(testCase) {
    ...
    // 执行回归检查
    const regressionResult = this.compareSnapshots(baseline, currentSnapshot, testCase);
  ...
  compareSnapshots(baseline, current, testCase) {
    ...
    if (testCase.criticalFields) {
      ...
      if (fieldDiff.length > 0) {
        differences.push(...fieldDiff.map(diff => ({
          ...diff,
          type: 'critical_field',
          severity: 'high'
        })));
    ...
    if (testCase.checkHeaders) {
      ...
      if (headerDiff.length > 0) {
        differences.push(...headerDiff.map(diff => ({
          ...diff,
          type: 'header',
          severity: 'low'
        })));
...
describe('API Regression Tests', () => {
    ...
    test(`API Regression: ${testCase.name}`, async () => {
      ...
      if (result.status === 'failed') {
        ...
        if (criticalIssues.length > 0) {
          ...
          console.warn('Non-critical differences detected:', result.differences);
        }
      }
  ...
  test('API Performance Regression', async () => {
    ...
    if (slowTests.length > 0) {
      ...
      // 如果配置了严格性能检查，则失败
      const strictTests = slowTests.filter(t => 
        performanceTests.find(pt => pt.name === t.testCase)?.strictPerformance
      );
      
      if (strictTests.length > 0) {
        throw new Error(`Performance regression in: ${strictTests.map(t => t.testCase).join(', ')}`);
      }
    }
  });
});
