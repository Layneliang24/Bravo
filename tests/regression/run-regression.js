...
class RegressionTestRunner {
  ...
  async runAll(options = {}) {
    ...
    try {
      ...
      console.log(
        `📊 Results: ${this.results.summary.passed} passed, ${this.results.summary.failed} failed, ${this.results.summary.errors} errors`,
      );
  ...
  async checkEnvironment() {
    ...
    try {
      ...
      throw new Error(
        "Configuration file not found: ./tests/regression/config/regression.config.js",
      );
    }
  ...
  async runDbTests(options = {}) {
    ...
    try {
      ...
      console.log(
        `✅ Database tests completed: ${this.results.db.length} tests`,
      );
    } catch (error) {
  ...
  buildJestCommand(testType, options = {}) {
    ...
    // 添加JSON输出用于解析结果
    command +=
      " --json --outputFile=./tests/regression/reports/jest-results.json";
...
<body>
    <div class="header">
        ...
        <p><strong>Environment:</strong> Node ${report.environment.node} on ${
    ...
    const testsHtml = results
      .flatMap((testFile) =>
        ...
        <div class="test-result test-${test.status}">
            ...
            <span style="float: right;">${test.status} (${
              test.duration
            }ms)</span>
