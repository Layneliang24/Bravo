...
class DbRegressionTester {
  ...
  async runDbTest(testCase) {
    const { name, type, query, tables, checkConstraints, checkIndexes } = testCase;
    ...
    try {
      ...
      // 执行回归检查
      const regressionResult = this.compareDbSnapshots(baseline, currentSnapshot, testCase);
  ...
  async captureData(query, tables = []) {
    ...
    if (query) {
      ...
      // 捕获指定表的数据
      for (const table of tables) {
        data[table] = await this.executeQuery(`SELECT * FROM ${table} ORDER BY id`);
      }
    } else {
      throw new Error('Either query or tables must be specified for data capture');
    }
  ...
  compareDbSnapshots(baseline, current, testCase) {
    ...
    if (structureDiff.length > 0) {
      differences.push(...structureDiff.map(diff => ({
        ...diff,
        type: 'structure',
        severity: 'high'
      })));
    ...
    if (testCase.type === 'data') {
      ...
      if (dataDiff.length > 0) {
        differences.push(...dataDiff.map(diff => ({
          ...diff,
          type: 'data',
    ...
    Object.keys(baseline).forEach(table => {
      if (current[table]) {
        ...
        if (Array.isArray(baselineData) && Array.isArray(currentData)) {
          ...
          // 比较记录内容（取样本）
          const sampleSize = Math.min(baselineData.length, currentData.length, 10);
...
describe('Database Regression Tests', () => {
    ...
    test(`DB Regression: ${testCase.name}`, async () => {
      ...
      if (result.status === 'failed') {
        ...
        if (criticalIssues.length > 0) {
          ...
          console.warn('Non-critical DB differences detected:', result.differences);
        }
      }
  ...
  test('Database Consistency Check', async () => {
    const consistencyTests = testCases.filter(tc => tc.type === 'data' && tc.strictData);
    ...
    if (failedTests.length > 0) {
      throw new Error(`Data consistency issues in: ${failedTests.map(t => t.testCase).join(', ')}`);
    }
  });
});
