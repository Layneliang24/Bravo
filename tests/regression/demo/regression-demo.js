...
class RegressionDemo {
  ...
  async runDemo() {
    ...
    try {
      ...
      console.log(
        chalk.yellow("📊 查看详细报告: tests/regression/demo/demo-report.html"),
      );
    } catch (error) {
  ...
  async runScenario(scenario) {
    ...
    try {
      ...
      console.error(
        chalk.red(`❌ 场景 ${scenario.name} 执行失败:`),
        error.message,
      );
    } finally {
      // 5. 恢复原文件
  ...
  async backupFiles(scenario) {
    ...
    try {
      ...
      console.log(
        chalk.yellow(`   ⚠️  无法备份文件: ${scenario.target} (可能不存在)`),
      );
    }
  }
    ...
    try {
      ...
      for (const change of scenario.changes) {
        if (content.includes(change.from)) {
          ...
          console.log(
            chalk.gray(
              `   ⏭️  跳过变更: ${change.description} (未找到目标代码)`,
            ),
          );
        }
      }
      ...
      console.log(
        chalk.yellow(
          `   ⚠️  无法应用变更到: ${scenario.target} (文件可能不存在)`,
        ),
      );
      // 创建模拟文件来演示
...
<body>
    <div class="container">
        <div class="header">
            ...
            <p class="timestamp">生成时间: ${new Date(
              report.timestamp,
