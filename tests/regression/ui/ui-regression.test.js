  ...
  async compareScreenshots(baseline, current, testCase) {
    try {
      ...
      const maxDiffPercentage =
        testCase.maxDiffPercentage || this.config.maxDiffPercentage || 1;
      ...
      return {
        passed,
        message: passed
          ? `UI regression test passed for ${
              testCase.name
            } (${diffPercentage.toFixed(2)}% difference)`
          : `UI regression detected for ${
              testCase.name
            }: ${diffPercentage.toFixed(
              2,
            )}% difference (threshold: ${maxDiffPercentage}%)`,
        differences: passed
          ? []
          : [
              {
                ...
                severity:
                  diffPercentage > maxDiffPercentage * 2 ? "high" : "medium",
              },
            ],
  ...
  test(`UI Regression: ${testCase.name}`, async ({ page }) => {
    ...
    if (result.status === "failed") {
      ...
      if (criticalIssues.length > 0) {
        ...
        console.warn(
          "Non-critical visual differences detected:",
          result.differences,
        );
      }
    }
  ...
  for (const testCase of responsiveTests) {
    for (const viewport of viewports) {
      ...
      if (result.status === "failed") {
        ...
        if (criticalIssues.length > 0) {
          throw new Error(
            `Responsive regression in ${viewport.name}: ${result.message}`,
          );
        }
      }
    }
...
test("UI Cross-browser Regression", async ({ page, browserName }) => {
  ...
  for (const testCase of crossBrowserTests) {
    ...
    if (result.status === "failed") {
      ...
      if (criticalIssues.length > 0) {
        throw new Error(
          `Cross-browser regression in ${browserName}: ${result.message}`,
        );
      }
    }
  }
