...
class FrontendBackendRegressionDemo {
    ...
    async runBackendRegressionTests() {
        ...
        for (const test of backendTests) {
            try {
                ...
                console.log(`  ${result.passed ? '✅' : '❌'} ${test.name}: ${result.details} (${result.responseTime}ms)`);
            } catch (error) {
                this.results.backend.push({
                    ...test,
                    status: 'ERROR',
    ...
    async runDatabaseRegressionTests() {
        ...
        const databaseTests = [
            ...
            },
            {
                ...
                test: () => this.simulateQueryPerformanceRegression('articles-published')
            },
            {
                name: '外键约束完整性',
        ...
        for (const test of databaseTests) {
            try {
                ...
                console.log(`  ${result.passed ? '✅' : '❌'} ${test.name}: ${result.details} (${result.executionTime}ms)`);
            } catch (error) {
                this.results.database.push({
                    ...test,
                    status: 'ERROR',
    ...
    async simulateQueryPerformanceRegression(query) {
        ...
        return {
            ...
            details: executionTime < threshold ? 
    ...
    generateSeparationReport() {
        const reportPath = path.join(this.baseDir, 'demo', 'separation-report.html');
...
<body>
    <div class="container">
        ...
        ${this.generateTestSection('🖥️ 前端UI回归测试结果', this.results.frontend, 'frontend')}
        ${this.generateTestSection('🔧 后端API回归测试结果', this.results.backend, 'backend')}
        ${this.generateTestSection('🗄️ 数据库回归测试结果', this.results.database, 'database')}
...
make test-regression-db</code></pre>
    ...
    generateTestSection(title, results, type) {
        const items = results.map(result => {
            ...
            const timeInfo = type === 'backend' ? ` (${result.responseTime}ms)` : 
                           type === 'database' ? ` (${result.executionTime}ms)` : '';
            
            return `
                <div class="test-item ${statusClass}">
                    <div><span class="icon">${icon}</span><strong>${result.name}</strong></div>
                    <div>${result.details}${timeInfo}</div>
                    <div class="test-meta">
                        ${type === 'frontend' ? `组件: ${result.component || 'N/A'} | 类型: ${result.type || 'N/A'}` : ''}
                        ${type === 'backend' ? `端点: ${result.endpoint || 'N/A'} | 方法: ${result.method || 'N/A'}` : ''}
                        ${type === 'database' ? `对象: ${result.table || result.query || result.constraint || result.index || 'N/A'}` : ''}
    ...
    async run() {
        ...
        const totalTests = this.results.frontend.length + this.results.backend.length + this.results.database.length;
        const totalPassed = [...this.results.frontend, ...this.results.backend, ...this.results.database]
