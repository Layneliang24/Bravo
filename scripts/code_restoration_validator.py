...
class CodeRestorationValidator:
    ...
    def validate_against_baseline(self) -> Dict:
        ...
        validation_result = {
            ...
            "feature_changes": self._compare_features(
                baseline["features"], current_state["features"]
            ),
        ...
        # 评估整体状态
        validation_result["overall_status"] = self._evaluate_overall_status(
            validation_result
        )
    ...
    def _run_backend_tests(self) -> Dict:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                test_count = len(
                    [line for line in output_lines if "✅" in line or "PASS" in line]
                )
                return {
                    "status": "passed",
                    "total": test_count,
            ...
            return {"status": "error", "total": 0, "passed": 0, "error": str(e)}
    ...
    def _run_e2e_tests(self) -> Dict:
        ...
        if e2e_config.exists():
            ...
            return {"status": "not_configured", "total": 0, "passed": 0}
    ...
    def _compare_features(
        self, baseline_features: List[Dict], current_features: List[Dict]
    ) -> Dict:
        """比较功能列表"""
        baseline_ids = {
            f.get("id", f.get("name", str(i))) for i, f in enumerate(baseline_features)
        }
        current_ids = {
            f.get("id", f.get("name", str(i))) for i, f in enumerate(current_features)
        }
    ...
    def _evaluate_overall_status(self, validation_result: Dict) -> str:
        ...
        if (
            test_changes["status"] == "degraded"
            and test_changes["pass_rate_change"] < -10
        ):
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "create-baseline":
        ...
        print(f"🧪 测试通过率: " f"{baseline['test_results']['summary']['pass_rate']:.1f}%")
        return 0
    ...
    # 保存报告
    report_file = (
        Path(project_root) / "docs" / "02_test_report" / "code_restoration_report.md"
    )
    ...
    # 根据状态返回退出码
    status_codes = {"healthy": 0, "attention": 0, "warning": 1, "critical": 2}
