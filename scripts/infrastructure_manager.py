                    ...
                    if lighthouse_dir.exists():
                        for report_file in lighthouse_dir.glob("lhr-*.json"):
                            try:
                                ...
                                logger.warning(f"Failed to parse Lighthouse report {report_file}: {e}")
...
def main():
    ...
    setup_parser.add_argument('--mode', choices=['docker', 'local'], default='docker', help='设置模式')
    ...
    test_parser.add_argument('--type', choices=['all', 'frontend', 'backend', 'e2e'], default='all', help='测试类型')
