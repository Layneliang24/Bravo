...
class MonitoringDashboard:
    ...
    def show_dashboard(self):
        ...
        if stats.get('total_checks', 0) > 0:
            block_rate = (stats.get('commits_blocked', 0) / stats.get('total_checks', 1)) * 100
        ...
        if daily_stats:
            ...
            for date in reversed(recent_days):
                ...
                print(f"  {date}: {day_stats.get('checks', 0)}次检查, {day_stats.get('blocks', 0)}次阻止")
        
        # 反馈统计
        print("\n💬 反馈统计:")
        if feedback_list:
            open_feedback = [f for f in feedback_list if f.get('status') == 'open']
            resolved_feedback = [f for f in feedback_list if f.get('status') == 'resolved']
    ...
    def show_feedback(self, status_filter: Optional[str] = None):
        ...
        for feedback in sorted(feedback_list, key=lambda x: x.get('timestamp', ''), reverse=True):
            status_icon = {
                'open': '🔴',
                'in_progress': '🟡', 
                'resolved': '🟢',
            ...
            print(f"ID: {feedback.get('id', 'N/A')} {status_icon} {severity_icon}")
    ...
    def export_report(self, output_file: str):
        ...
        report = {
            ...
            'summary': {
                ...
                'success_rate': ((stats.get('commits_allowed', 0) / max(stats.get('total_checks', 1), 1)) * 100),
                'average_scan_time': stats.get('average_scan_time', 0),
                'open_feedback_count': len([f for f in feedback_list if f.get('status') == 'open']),
                'health_issues': len([h for h in health_status.values() if h['status'] == 'error'])
            }
        }
...
def main():
    ...
    export_parser.add_argument('--output', default='monitoring_report.json', help='输出文件名')
