...
class PreCommitMonitor:
            ...
            if result.returncode in [0, 2]:  # 0=无问题, 2=发现问题但正常
                ...
                for line in output_lines:
                    if "高风险" in line and "处" in line:
                        ...
                        try:
                            medium_risk = int(
                                line.split("中风险")[1].split("处")[0].strip()
                            )
                        except Exception:  # nosec
                            pass
                        ...
                        try:
                            total_modifications = int(
                                line.split("发现")[1].split("处临时修改")[0].strip()
                            )
                        except Exception:  # nosec
                            pass
    ...
    def _run_tests(self) -> Dict:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                return {"status": "failed", "error": result.stderr, "pass_rate": 0.0}
        except Exception as e:
            return {"status": "error", "error": str(e), "pass_rate": 0.0}
    ...
    def _check_feature_integrity(self) -> Dict:
        ...
        if not features_file.exists():
            return {"status": "warning", "message": "features.json文件不存在"}
    ...
    def _assess_commit_risk(self, results: Dict) -> Dict:
        ...
        if critical_files.get("missing_files"):
            risk_factors.append(
                f'关键文件缺失: {len(critical_files.get("missing_files", []))}'
            )
            risk_score += 3

        if critical_files.get("modified_files"):
            risk_factors.append(
                f'关键文件被修改: {len(critical_files.get("modified_files", []))}'
            )
            risk_score += 1
    ...
    def generate_commit_report(self, results: Dict) -> str:
        ...
        lines.append(
            f"{recommendation_icons.get(recommendation, '❓')} "
            f"**建议**: {recommendation}"
        )
        ...
        if feature_integrity.get("status") == "success":
            ...
            message = feature_integrity.get(
                "message", feature_integrity.get("error", "Unknown")
            )
        ...
        if not critical_files.get("missing_files") and not critical_files.get(
            "modified_files"
        ):
