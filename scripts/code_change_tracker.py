...
class CodeChangeTracker:
    ...
    def _generate_summary(self, results: Dict) -> Dict:
        """生成扫描摘要"""
        summary = {
            'total_issues': len(results['temporary_markers']) + len(results['high_risk_changes']),
            'high_severity_count': len([x for x in results['temporary_markers'] if x['severity'] == 'HIGH']),
    ...
    def compare_with_previous(self, current_data: Dict) -> Dict:
        ...
        if curr_risk_count > prev_risk_count:
            changes.append({
                ...
                'message': f'高风险变更增加了 {curr_risk_count - prev_risk_count} 个',
                'severity': 'HIGH'
            })
        ...
        return {
            'status': 'compared',
            'changes': changes,
            'trend': 'improving' if len(changes) == 0 or all(c['type'] == 'DECREASE' for c in changes) else 'degrading'
        }
    ...
    def generate_report(self, data: Dict, comparison: Dict) -> str:
        ...
        if data['high_risk_changes']:
            ...
            for change in data['high_risk_changes'][:10]:  # 只显示前10个
                report.append(f"- **{change['file']}:{change['line']}** - {change['type']}")
                report.append(f"  ```{change['content']}```")
        
        # 临时标记详情
        if data['temporary_markers']:
            report.append("\n## ⚠️ 临时标记")
            high_severity = [x for x in data['temporary_markers'] if x['severity'] == 'HIGH']
            for marker in high_severity[:10]:  # 只显示前10个高严重性
                report.append(f"- **{marker['file']}:{marker['line']}** - {marker['type']}")
...
def main():
    ...
    # 保存报告
    report_file = Path(project_root) / 'docs' / '02_test_report' / 'code_change_tracking_report.md'
