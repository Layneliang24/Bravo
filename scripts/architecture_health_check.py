...
class ArchitectureHealthChecker:
    ...
    def check_npm_workspaces_violations(self):
        ...
        compliance_rate = (
            ((total_files - violation_files) / total_files * 100)
            if total_files > 0
            else 100
        )
    ...
    def check_complexity_metrics(self):
        ...
        # package.json文件数量
        package_jsons = len(
            [p for p in Path(".").rglob("package.json") if "node_modules" not in str(p)]
        )
        ...
        complexity_score = min(
            100, (workflows_count * 2) + (docker_configs * 5) + (package_jsons * 3)
        )
            ...
            recommendations.append(
                {
                    ...
                    "description": (
                        f"工具链复杂度过高 " f'(评分: {complexity.get("complexity_score", 0)})'
                    ),
            ...
            recommendations.append(
                {
                    ...
                    "description": (
                        f"GitHub Actions工作流过多 "
                        f'({complexity.get("workflows_count")} 个)'
                    ),
    ...
    def generate_summary(self):
        ...
        compliance_rate = (
            self.report["metrics"].get("npm_workspaces", {}).get("compliance_rate", 0)
        )
        complexity_level = (
            self.report["metrics"]
            .get("complexity", {})
        ...
        status = (
            "HEALTHY"
            if violations == 0 and compliance_rate == 100
