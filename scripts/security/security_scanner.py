    ...
    def run_safety_scan(self) -> List[SecurityIssue]:
        ...
        try:
            ...
            result = subprocess.run(
                cmd, capture_output=True, text=True, cwd=self.project_root / "backend"
            )
    ...
    def run_npm_audit_scan(self) -> List[SecurityIssue]:
        ...
        for js_dir in ["frontend", "e2e"]:
            ...
            try:
                ...
                cmd = ["npm", "audit", "--json"]
                result = subprocess.run(
                    cmd, capture_output=True, text=True, cwd=js_path
                )
                    ...
                    try:
                        ...
                        for vuln_id, vuln in audit_data.get(
                            ...
                            issue = SecurityIssue(
                                ...
                                if isinstance(vuln.get("url"), list)
                                else [vuln.get("url")]
                                if vuln.get("url")
                                else None,
                            )
                ...
                logger.info(
                    f"npm audit found {len([i for i in issues if js_dir in str(i.file_path)])} issues in {js_dir}"
                )
        ...
        for pattern_name, pattern in secret_patterns.items():
            for file_path in self.project_root.rglob("*"):
                if (
                    ...
                    in [".py", ".js", ".ts", ".json", ".yml", ".yaml", ".env", ".txt"]
                ):
                    try:
                ...
                if config_file.is_file():
                    try:
                        ...
                        if any(
                            ...
                            if not any(
                                safe in content for safe in ["${", "env:", "environ"]
                            ):
                                issue = SecurityIssue(
                                    severity="high",
                                    category="config",
...
def main():
    ...
    # 输出摘要
    print("\n[SECURITY] Security Scan Summary:")
    ...
    if report.critical_count > 0 or report.high_count > 0:
        print("\n[WARNING] High priority security issues found!")
        exit(1)
    else:
