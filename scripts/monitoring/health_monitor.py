...
class HealthMonitor:
    ...
    def check_all_services(self) -> List[ServiceHealth]:
        ...
        for service_name, service_config in self.services.items():
            ...
            logger.info(f"{service_name}: {health.status} ({health.response_time:.2f}ms)")
    ...
    def analyze_health(self, services: List[ServiceHealth], metrics: SystemMetrics) -> Dict[str, Any]:
        """分析健康状态并生成报告"""
        ...
        elif (metrics.cpu_percent > self.thresholds['cpu_critical'] or
    ...
    def _generate_recommendations(self, services: List[ServiceHealth], metrics: SystemMetrics) -> List[str]:
        """生成改进建议"""
        recommendations = []
        ...
        for service in services:
            if service.status == 'critical':
                ...
                recommendations.append(f"⚠️ {service.name} 响应时间过长({service.response_time:.0f}ms)，建议优化")
