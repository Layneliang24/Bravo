...
try:
    from production_config import (
        HIGH_SEVERITY_PATTERNS, MEDIUM_SEVERITY_PATTERNS, LOW_SEVERITY_PATTERNS,
        SUPPORTED_EXTENSIONS, EXCLUDED_PATHS, EXCLUDED_FILES,
        MAX_FILE_SIZE, SEVERITY_THRESHOLDS, MESSAGE_TEMPLATES,
        ENABLE_STATISTICS, STATISTICS_FILE, initialize_config
    )
    initialize_config()
except ImportError:
    # 回退到基础配置
...
class FastPreCommitChecker:
    ...
    def update_statistics(self, result: Dict):
        ...
        if self.stats['average_scan_time'] == 0:
            ...
            self.stats['average_scan_time'] = (
                self.stats['average_scan_time'] * (self.stats['total_checks'] - 1) + duration
    ...
    def get_staged_files(self) -> List[str]:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                # 只检查代码文件
                code_extensions = {'.py', '.js', '.ts', '.vue', '.jsx', '.tsx', '.java', '.cpp', '.c', '.h'}
    ...
    def run_fast_check(self) -> Tuple[bool, Dict]:
        ...
        is_safe = self._evaluate_safety(high_severity_count, medium_severity_count, low_severity_count, total_issues)
    ...
    def _evaluate_safety(self, high_count: int, medium_count: int, low_count: int, total_count: int) -> bool:
        """根据生产配置评估提交安全性"""
        # 检查高严重性问题
        high_threshold = self.severity_thresholds.get('high', {})
        if high_threshold.get('block_commit', True) and high_count > high_threshold.get('max_issues', 0):
            return False
        
        # 检查中等严重性问题
        medium_threshold = self.severity_thresholds.get('medium', {})
        if medium_threshold.get('block_commit', False) and medium_count > medium_threshold.get('max_issues', 5):
            return False
        
        # 检查低严重性问题
        low_threshold = self.severity_thresholds.get('low', {})
        if low_threshold.get('block_commit', False) and low_count > low_threshold.get('max_issues', 20):
            return False
    ...
    def _print_results(self, result: Dict):
        ...
        print(self.message_templates['issues_found'].format(
        ...
        if result['issues']:
            ...
            for issue in result['issues']:
                ...
                print(f"  {severity_icon} {issue['file']}:{issue['line']} - {content}")
                displayed_count += 1
        ...
        if should_block:
            print(f"\n{self.message_templates['commit_blocked'].format(count=high_count)}")
            print(self.message_templates['bypass_hint'])
        else:
            print(f"\n{self.message_templates['commit_allowed']}")
            if medium_count > 0 or low_count > 0:
                print(f"[INFO] 注意: 发现 {medium_count + low_count} 个非关键问题，建议后续处理")
    ...
    try:
        ...
        if ENABLE_STATISTICS and hasattr(checker, 'stats'):
            ...
            if stats['total_checks'] > 1:  # 不是第一次运行
                print(f"[STATS] 历史统计: 总检查 {stats['total_checks']} 次, "
