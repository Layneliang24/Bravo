...
class TempModificationDetector:
    ...
    def __init__(self, project_root: str):
        ...
        self.detection_patterns = {
            ...
            'COMMENTED_CODE': {
                'pattern': r'^\s*#\s*(def |class |import |from |if |for |while |try |except |return |print\()',
                'severity': 'medium',
                'description': '被注释的代码'
            },
            'CONSOLE_LOG': {
            ...
            'PRINT_DEBUG': {
                'pattern': r'print\s*\([^)]*debug|print\s*\([^)]*test|print\s*\([^)]*temp',
                'severity': 'low',
                'description': '调试打印语句'
            }
        }
    ...
    def _check_line(self, file_path: Path, line_num: int, line: str, all_lines: List[str]) -> List[TempModification]:
        """检查单行代码"""
        modifications = []
    ...
    def generate_markdown_report(self, report: Dict) -> str:
        ...
        for severity in ['high', 'medium', 'low']:
            ...
            for mod in modifications:
                lines.append(f"\n### {mod['marker_type']} - {mod['file_path']}:{mod['line_number']}")
        ...
        if report['modifications_by_file']:
            ...
            for file_path, modifications in report['modifications_by_file'].items():
                ...
                for mod in modifications:
                    lines.append(f"- 第{mod['line_number']}行: {mod['marker_type']} - {mod['description']}")
