...
function findTestFiles() {
  ...
  for (const pattern of CONFIG.testPatterns) {
    try {
      ...
      console.warn(`⚠️  Failed to find files with pattern ${pattern}:`, error.message);
    }
  }
...
function parseTestFile(filePath) {
  try {
    ...
    const linkMatches = content.match(/linkTestToFeature\s*\(\s*['"`]([^'"`)]+)['"`]\s*\)/g);
    if (linkMatches) {
    ...
    const describeMatches = content.match(/describeFeature\s*\(\s*['"`]([^'"`)]+)['"`]/g);
    if (describeMatches) {
...
function generateMarkdownReport(featureTestMap, features) {
  ...
  const coveredFeatures = Array.from(featureTestMap.values()).filter(item => item.tests.size > 0).length;
  const coveragePercentage = totalFeatures > 0 ? (coveredFeatures / totalFeatures * 100).toFixed(2) : 0;
  ...
  Object.entries(categoryStats).forEach(([category, stats]) => {
    const percentage = stats.total > 0 ? (stats.covered / stats.total * 100).toFixed(1) : 0;
    lines.push(`| ${category} | ${stats.total} | ${stats.covered} | ${percentage}% |`);
  });
  lines.push('');
  ...
  features.forEach(feature => {
    ...
    const testFilesList = testFiles.length > 0 ? testFiles.map(f => `\`${f}\``).join('<br>') : '—';
    
    lines.push(`| ${feature.id} | ${feature.desc} | ${feature.category || 'N/A'} | ${feature.priority || 'N/A'} | ${testFilesList} | ${status} |`);
  });
  
  // 未覆盖功能
  const uncoveredFeatures = features.filter(feature => featureTestMap.get(feature.id).tests.size === 0);
  if (uncoveredFeatures.length > 0) {
    lines.push('\n## ❌ 未覆盖功能\n');
    uncoveredFeatures.forEach(feature => {
      lines.push(`- **${feature.id}**: ${feature.desc} (${feature.category || 'N/A'}, ${feature.priority || 'N/A'})`);
    });
  }
  
  // 高优先级未覆盖
  const highPriorityUncovered = uncoveredFeatures.filter(f => f.priority === 'high' || f.priority === 'critical');
...
// 生成JSON报告
function generateJsonReport(featureTestMap, testFeatureMap, features, testFiles) {
  const report = {
    timestamp: new Date().toISOString(),
    summary: {
      totalFeatures: features.length,
      totalTestFiles: testFiles.length,
      coveredFeatures: Array.from(featureTestMap.values()).filter(item => item.tests.size > 0).length,
  ...
  report.summary.coveragePercentage = report.summary.totalFeatures > 0 
...
function main() {
  try {
    ...
    const { featureTestMap, testFeatureMap, features, testFiles } = buildFeatureTestMap();
    ...
    // 生成JSON报告
    const jsonReport = generateJsonReport(featureTestMap, testFeatureMap, features, testFiles);
    fs.writeFileSync(CONFIG.jsonOutputFile, JSON.stringify(jsonReport, null, 2));
    ...
    if (process.env.CI && coveragePercentage < minCoverage) {
      console.error(`\n💥 Feature coverage ${coveragePercentage.toFixed(2)}% is below minimum ${minCoverage}%`);
      process.exit(1);
    }
