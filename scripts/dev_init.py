...
class DevEnvironmentInitializer:
    ...
    def create_baseline_if_needed(self) -> dict:
        ...
        if self.check_baseline_exists():
            ...
            return {
                'status': 'exists',
                'message': f"基线已存在，创建时间: {baseline.get('timestamp', 'unknown')}",
                'baseline': baseline
            }
        ...
        try:
            result = subprocess.run(
                [sys.executable, str(self.scripts_dir / 'comprehensive_code_manager.py'), 'create-baseline'],
                cwd=self.project_root,
                capture_output=True,
                text=True
            )
    ...
    def setup_git_hooks(self) -> dict:
        ...
        try:
            ...
            return {
                'status': 'manual_setup',
                'message': '请手动配置Git钩子或运行: npx husky add .husky/pre-commit "python scripts/pre_commit_monitor.py"'
            }
            
        except Exception as e:
            return {
                'status': 'error',
                'message': f'设置Git钩子时出错: {str(e)}'
            }
    ...
    def run_initial_scan(self) -> dict:
        ...
        try:
            result = subprocess.run(
                [sys.executable, str(self.scripts_dir / 'temp_modification_detector.py')],
                cwd=self.project_root,
                capture_output=True,
                text=True
            )
