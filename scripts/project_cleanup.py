...
class ProjectCleaner:
            ...
            for file_path in self.project_root.rglob(pattern):
                if file_path.is_file() and not self.is_protected(file_path):
                    try:
                        ...
                        logger.info(f"删除占位文件: {file_path.relative_to(self.project_root)}")
                ...
                for file_path in self.project_root.rglob(pattern):
                    if not self.is_protected(file_path):
                        try:
                            ...
                            removed_files.append(file_path)
                            logger.info(f"删除过时文件: {file_path.relative_to(self.project_root)}")
                        except Exception as e:
                            logger.error(f"删除失败 {file_path}: {e}")
    ...
    def run_full_cleanup(self) -> Dict:
        ...
        result = {
            ...
            "removed_files": [str(f.relative_to(self.project_root)) for f in all_removed_files],
