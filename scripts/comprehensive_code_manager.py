...
class ComprehensiveCodeManager:
    ...
    def __init__(self, project_root: str):
        ...
        self.detection_patterns = {
            ...
            'DEBUG_PRINT': {
                'pattern': r'print\s*\([^)]*(?:debug|test|temp|DEBUG|TEST|TEMP)[^)]*\)',
                'severity': 'low',
                'description': '调试打印语句'
            },
            'CONSOLE_DEBUG': {
                'pattern': r'console\.(log|debug)\s*\([^)]*(?:debug|test|temp|DEBUG|TEST|TEMP)[^)]*\)',
                'severity': 'low',
                'description': '调试控制台输出'
            },
            'COMMENTED_FUNCTION': {
        ...
        self.exclude_patterns = {
            ...
            '.vscode', '.idea', '.code_baselines', 'htmlcov',
            '.mypy_cache', 'venv', 'env',
            # 文件
    ...
    def scan_for_issues(self) -> List[CodeIssue]:
        ...
        for root, dirs, files in os.walk(self.project_root):
            dirs[:] = [d for d in dirs if not any(pattern in d for pattern in self.exclude_patterns if not pattern.startswith('r'))]
        ...
        for root, dirs, files in os.walk(self.project_root):
            ...
            for filename in files:
                ...
                if scanned_files % 100 == 0:
                    ...
                    print(f"[INFO] 已扫描 {scanned_files}/{total_files} 文件 ({elapsed:.2f}s)")
                
                file_issues = self._scan_file_for_issues(file_path)
                issues.extend(file_issues)
        
        elapsed_time = time.time() - start_time
        print(f"[SUCCESS] 扫描完成: {scanned_files} 文件, {len(issues)} 问题, 耗时 {elapsed_time:.2f}s")
    ...
    def _scan_file_for_issues(self, file_path: Path) -> List[CodeIssue]:
        ...
        try:
            ...
            for line_num, line in enumerate(lines, 1):
                line_issues = self._check_line_for_issues(file_path, line_num, line, lines)
    ...
    def _check_line_for_issues(self, file_path: Path, line_num: int, line: str, all_lines: List[str]) -> List[CodeIssue]:
        """检查单行代码问题"""
        issues = []
        ...
        for issue_type, config in self.detection_patterns.items():
            ...
            if match:
                ...
                # 提取描述
                description = match.group(1) if match.groups() else config['description']
    ...
    def validate_current_state(self) -> Dict:
        ...
        validation = {
            ...
            'features_analysis': self._analyze_features_change(
                baseline['features_count'], 
                current_state['features_count']
            ),
            ...
            'issues_analysis': self._analyze_issues_change(
                baseline['code_issues'], 
                current_state['code_issues']
            ),
            'files_analysis': self._analyze_files_change(
                baseline['file_checksums'], 
                current_state['file_checksums']
            )
        }
    ...
    def _analyze_test_change(self, baseline_tests: Dict, current_tests: Dict) -> Dict:
        ...
        return {
            ...
            'status': 'improved' if current_rate > baseline_rate else 
                     'degraded' if current_rate < baseline_rate else 'stable'
        }
    
    def _analyze_issues_change(self, baseline_issues: List[Dict], current_issues: List[Dict]) -> Dict:
        """分析代码问题变更"""
        ...
        return {
            ...
            'status': 'improved' if current_high < baseline_high else 
                     'degraded' if current_high > baseline_high else 'stable'
        }
    
    def _analyze_files_change(self, baseline_checksums: Dict, current_checksums: Dict) -> Dict:
        """分析文件变更"""
        changed_files = []
...
def main():
    ...
    if validation.get('status') == 'no_baseline':
        print("[ERROR] 未找到基线，请先运行: python comprehensive_code_manager.py create-baseline")
        return 1
