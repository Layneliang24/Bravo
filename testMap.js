...
function validateFeatureId(featureId) {
  ...
  if (!validIds.includes(featureId)) {
    throw new Error(`Invalid feature ID: ${featureId}. Valid IDs: ${validIds.join(', ')}`);
  }
...
function linkTestToFeature(featureId, testFile = null) {
  ...
  if (!testFile) {
    throw new Error('Could not determine test file. Please provide testFile parameter.');
  }
...
function getFeatureCoverage(featureId = null) {
  ...
  if (featureId) {
    ...
    return {
      ...
      covered: featureTestMap.has(featureId) && featureTestMap.get(featureId).size > 0
    };
  }
  ...
  return features.map(feature => ({
    ...
    covered: featureTestMap.has(feature.id) && featureTestMap.get(feature.id).size > 0
  }));
}
...
function validateAllFeaturesCovered() {
  ...
  for (const feature of features) {
    if (!featureTestMap.has(feature.id) || featureTestMap.get(feature.id).size === 0) {
      uncoveredFeatures.push(feature);
    }
  }
  
  if (uncoveredFeatures.length > 0) {
    const errorMsg = `Uncovered features found:\n${uncoveredFeatures.map(f => `  - ${f.id}: ${f.desc}`).join('\n')}`;
...
function generateCoverageReport() {
  ...
  const report = {
    ...
    uncovered: coverage.filter(c => !c.covered).map(c => ({
      featureId: c.feature.id,
      description: c.feature.desc,
      category: c.feature.category,
      priority: c.feature.priority
